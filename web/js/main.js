...
$(document).ready(function(){ 	
	//panier reduction js
$('#flshr_gestionpublicationbundle_commandetype_reduction').on('change', function() {
var x = $('#ttc-label').text() - (($(this).val()/100)*($('#ttc-label').text()));
$('#reduction-label').text(x); 
$('#total-label').text(x); 
$('#total-label-top').text(x); 
$('#total-label').shuffleLetters({
    "fps": 40
});
});    
	$('.lightbox').lightbox();
